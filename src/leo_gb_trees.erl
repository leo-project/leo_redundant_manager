...
lookup_1(_, {Key, Val, _, _},_SoFar) ->
    {Key, Val};
lookup_1(_, nil, SoFar) ->
    SoFar.

%% @private
lookup_2(Key, {K1,_V1}, {K2,V2,_,_}) when Key < K2,
                                          K1  > K2 ->
    {K2,V2};
lookup_2(_Key, {K1,V1}, _) ->
    {K1,V1}.

%% @private
lookup_3(nil, {K,V}) ->
    {K,V};
lookup_3({K1,V1},{K2,_V2}) when K1 < K2 ->
    {K1,V1};
lookup_3(_,{K,V}) ->
    {K,V}.
...
is_defined_1(Key, {Key_1, _, Smaller, _}) when Key < Key_1 ->
    is_defined_1(Key, Smaller);
is_defined_1(Key, {Key_1, _, _, Bigger}) when Key > Key_1 ->
...
get_1(Key, {Key_1, _, Smaller, _}) when Key < Key_1 ->
    get_1(Key, Smaller);
get_1(Key, {Key_1, _, _, Bigger}) when Key > Key_1 ->
    get_1(Key, Bigger);
get_1(_, {_, Value, _, _}) ->
    Value.
...
update_1(Key, Value, {Key_1, V, Smaller, Bigger}) when Key < Key_1 ->
    {Key_1, V, update_1(Key, Value, Smaller), Bigger};
update_1(Key, Value, {Key_1, V, Smaller, Bigger}) when Key > Key_1 ->
    {Key_1, V, Smaller, update_1(Key, Value, Bigger)};
...
delete_1(Key, {Key_1, Value, Smaller, Larger}) when Key < Key_1 ->
    Smaller1 = delete_1(Key, Smaller),
    {Key_1, Value, Smaller1, Larger};
delete_1(Key, {Key_1, Value, Smaller, Bigger}) when Key > Key_1 ->
    Bigger1 = delete_1(Key, Bigger),
    {Key_1, Value, Smaller, Bigger1};
...
take_smallest1({Key, Value, Smaller, Larger}) ->
    {Key_1, Value1, Smaller1} = take_smallest1(Smaller),
    {Key_1, Value1, {Key, Value, Smaller1, Larger}}.
...
take_largest1({Key, Value, Smaller, Larger}) ->
    {Key_1, Value1, Larger1} = take_largest1(Larger),
    {Key_1, Value1, {Key, Value, Smaller, Larger1}}.
